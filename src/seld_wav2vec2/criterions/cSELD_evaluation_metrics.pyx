...
class SELDMetrics(object):
    def __init__(self, int doa_threshold=20, int nb_classes=11):
        """
    ...
    def compute_seld_scores(self):
        ...
        cdef float ER = (self._S + self._D + self._I) / float(self._Nref + eps)

        cdef float prec = float(self._TP) / float(self._Nsys + eps)
        cdef float recall = float(self._TP) / float(self._Nref + eps)
        cdef float F = 2 * prec * recall / (prec + recall + eps)
        ...
        cdef float DE_prec = float(self._DE_TP) / float(self._Nsys + eps)
        cdef float DE_recall = float(self._DE_TP) / float(self._Nref + eps)
        cdef float DE_F = 2 * DE_prec * DE_recall / (DE_prec + DE_recall + eps)
    ...
    def update_seld_scores_xyz(self, pred, gt):
        ...
        for block_cnt in range(len(gt.keys())):
            ...
            for class_cnt in range(self._nb_classes):
                ...
                # Counting the number of ref and sys outputs should include the number of tracks for each class in the
                # segment
                ...
                if class_cnt in gt[block_cnt] and class_cnt in pred[block_cnt]:
                    ...
                    # calculate the spatial distance using the following code. In the current code, if there are
                    # multiple
                    # tracks of the same class in a frame we are calculating the least cost between the groundtruth and
                    # predicted and using it.
                    ...
                    for gt_ind, gt_val in enumerate(gt_ind_list):
                        if gt_val in pred_ind_list:
                            ...
                            if gt_arr.shape[0] == 1 and pred_arr.shape[0] == 1:
                                ...
                                total_spatial_dist += least_distance_between_gt_pred(
                                    gt_arr, pred_arr
                                )
                    ...
                    class_cnt not in gt[block_cnt] and class_cnt not in pred[block_cnt]
                ):
                    # True negative
                    self._TN += 1
    ...
    def update_seld_scores(self, pred_deg: Dict, gt_deg: Dict):
        """
                ...
                if class_cnt in gt_deg[block_cnt] and class_cnt in pred_deg[block_cnt]:
                    ...
                    for gt_ind, gt_val in enumerate(gt_ind_list):
                        if gt_val in pred_ind_list:
                            ...
                            gt_arr = (
                                np.array(gt_deg[block_cnt][class_cnt][0][1][gt_ind])
                                * np.pi
                                / 180
                            )
                            pred_arr = (
                                np.array(pred_deg[block_cnt][class_cnt][0][1][pred_ind])
                                * np.pi
                                / 180
                            )
                            if gt_arr.shape[0] == 1 and pred_arr.shape[0] == 1:
                    ...
                    class_cnt in gt_deg[block_cnt]
                    and class_cnt not in pred_deg[block_cnt]
                ):
                    # False negative
                    loc_FN += 1
                    self._FN += 1
                elif (
                    class_cnt not in gt_deg[block_cnt]
                    and class_cnt in pred_deg[block_cnt]
                ):
                    # False positive
                    loc_FP += 1
                    self._FP += 1
...
def distance_between_spherical_coordinates_rad(float az1, float ele1, float az2, float ele2):
    """
    ...
    """
    cdef float dist = np.sin(ele1) * np.sin(ele2) + np.cos(ele1) * np.cos(ele2) * np.cos(
        np.abs(az1 - az2)
    )
...
def distance_between_cartesian_coordinates(float x1, float y1, float z1, float x2, float y2, float z2):
    """
    ...
    cdef float N1 = np.sqrt(x1**2 + y1**2 + z1**2 + 1e-10)
    cdef float N2 = np.sqrt(x2**2 + y2**2 + z2**2 + 1e-10)
    x1, y1, z1, x2, y2, z2 = x1 / N1, y1 / N1, z1 / N1, x2 / N2, y2 / N2, z2 / N2

    # Compute the distance
    cdef float dist = x1 * x2 + y1 * y2 + z1 * z2
    dist = np.clip(dist, -1, 1)
    dist = np.arccos(dist) * 180 / np.pi
    return dist
...
def least_distance_between_gt_pred(gt_list, pred_list):
    ...
    """
    cdef int gt_len = gt_list.shape[0]
    cdef int pred_len = pred_list.shape[0]
    ind_pairs = np.array([[x, y] for y in range(pred_len) for x in range(gt_len)])
    cdef cnp.ndarray cost_mat = np.zeros((gt_len, pred_len))
    ...
    cdef float cost = cost_mat[row_ind, col_ind].sum()
    return cost


def early_stopping_metric(sed_error: Tuple[float, float], doa_error: Tuple[float, float]):
    """
    ...
    """
    cdef float seld_metric = np.mean(
