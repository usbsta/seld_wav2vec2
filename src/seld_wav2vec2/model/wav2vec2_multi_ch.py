...
def split_channel_by_edges(channels, edges):
    """
    Splits into multiple parts based on the provided list of inner edges.

    Parameters:
        array: The input array to split.
        edges: A list of indices where the array should be split.

    Returns:
        A list of subarrays split at the specified edges.
    """

    array = list(range(channels))

    edges = sorted(edges)  # Ensure edges are sorted
    if edges[-1] > len(array):
        raise ValueError("One of the edges is larger than the array size.")

    # Use slicing to split the array at each edge
    parts = []
    start = 0
    for edge in edges:
        parts.append(array[start:edge])
        start = edge
    parts.append(array[start:])  # Add the remaining part
    return parts
...
class Wav2Vec2ChSpecConfig(Wav2Vec2ChConfig):
    ...
    learn_norm_input: bool = field(
        default=False, metadata={"help": "learnable norm input spec in CNN"}
    )
...
class Wav2Vec2ChModel(Wav2Vec2Model):
    def __init__(self, cfg: Wav2Vec2ChConfig):
        ...
        self.feature_extractor = ConvFeatureExtractionChModel(
            ...
            learn_norm_input=cfg.get("learn_norm_input", False),
        )
        ...
        if cfg.spectrogram_1d:
            ...
            self.embed = (
                ...
                * self._get_feat_extract_output_lengths(
                    torch.tensor(cfg.n_mels)
                ).tolist()
            )

        if cfg.conv_skip_connection:
            if cfg.channels_edges is not None:
                self.feature_extractor = ResnetFeatureExtractionChMultiModel(
                    in_channels=cfg.in_channels,
                    conv_layers=feature_enc_layers,
                    dropout=0.0,
                    mode=cfg.extractor_mode,
                    ...
                    conv_groups=cfg.in_conv_groups,
                    learn_norm_input=cfg.get("learn_norm_input", False),
                    conv1d=cfg.spectrogram_1d,
                    inner_channels_edges=cfg.channels_edges,
                )
            else:
            ...
            if cfg.spectrogram_1d:
                if cfg.channels_edges is not None:
                    self.feature_extractor = ConvFeatureExtractionChMultiModel(
                        in_channels=cfg.in_channels,
                        conv_layers=feature_enc_layers,
                        dropout=0.0,
                        mode=cfg.extractor_mode,
                ...
                if cfg.channels_edges is not None:
                    self.feature_extractor = Conv2DFeatureExtractionChMultiModel(
                        in_channels=cfg.in_channels,
                        conv_layers=feature_enc_layers,
                        dropout=0.0,
                        mode=cfg.extractor_mode,
...
class Wav2Vec2ChSpecMlmModel(Wav2Vec2ChModel):
    ...
    def forward(
        ...
        x_mlm, layer_results = self.encoder(x, padding_mask=padding_mask, layer=layer)
        ...
        assert len(layer_results) == self.cfg.encoder_layers, (
            f"length of layer_results {len(layer_results)}"
        )
        # get the output of the N-1 layer
        x = layer_results[self.cfg.num_constrastive_layers - 1][2].transpose(0, 1)
        ...
        # w2v 2.0 context vectors
        x = self.final_proj(x)
        x = self.compute_preds(x, y, negs)

        # mlm (bert) context vectors
    ...
    def joint_lm_labels(self, x):
        powers = torch.tensor(
            [
                self.cfg.latent_vars**i
                for i in range(self.cfg.latent_groups - 1, -1, -1)
            ],
            device=x.device,
        )
    ...
    def get_mlm_targets(self, net_output):
        mlm_targets = (
            net_output["mlm_targets"].reshape(-1, self.cfg.latent_groups).long()
        )
        return mlm_targets

    def get_mlm_logits(self, net_output):
        mlm_logits = (
            net_output["mlm_logits"]
            .reshape(-1, self.cfg.latent_vars, self.cfg.latent_groups)
            .float()
        )
        return mlm_logits


@register_model("wav2vec2_ch_spec_lm", dataclass=Wav2Vec2ChSpecMlmConfig)
class Wav2Vec2ChLMModel(Wav2Vec2ChModel):
    def __init__(self, cfg: Wav2Vec2ChSpecMlmConfig):
        BaseFairseqModel.__init__(self)
        self.cfg = cfg
        ...
        # constrastive learning (cl) and lm encoder
        self.encoder = encoder_cls(cfg)
    ...
    def forward(
        ...
        # lm transformer output
        x_lm, layer_results = self.encoder(x, padding_mask=padding_mask, layer=layer)
        ...
        # labels of lm (gpt)
        qt = self.quantizer(unmasked_features, produce_targets=True)
        lm_targets = qt["targets"]

        # lm (gpt) context vectors
        lm_logits = self.lm_proj(x_lm)
    ...
    def get_lm_targets(self, net_output):
        lm_targets = net_output["lm_targets"].reshape(-1, self.cfg.latent_groups).long()
        return lm_targets

    def get_lm_logits(self, net_output):
        lm_logits = (
    ...
    def build_model(cls, cfg: Wav2Vec2ChPretConfig, task=None):
        ...
        if cfg.pre_w2v_args is None:
            state = checkpoint_utils.load_checkpoint_to_cpu(
                cfg.pre_w2v_path, arg_overrides
            )
...
class Wav2Vec2ChSpecPretConfig(Wav2Vec2ChPretConfig):
    ...
    n_mels: int = II("task.n_mels")
    learn_norm_input: bool = field(
        default=False, metadata={"help": "batch norm input spec in CNN"}
    )
...
@register_model("wav2vec2_ch_spec_lm_pretr", dataclass=Wav2Vec2ChSpecMlmPretConfig)
class Wav2Vec2ChLMModelPtrained(Wav2Vec2ChLMModel):
    def __init__(self, cfg: Wav2Vec2ChSpecMlmPretConfig):
        super().__init__(cfg)
...
class ConvFeatureExtractionChModel(nn.Module):
    def __init__(
        ...
        conv_groups: int = 1,
        learn_norm_input: bool = False,
    ):
        super().__init__()

        if learn_norm_input:
            self.norm_input = GlobalInstanceNorm1d(in_channels, affine=True)
        else:
        ...
        def block(
            ...
            def make_conv():
                conv = nn.Conv1d(
                    n_in, n_out, k, stride=stride, bias=conv_bias, groups=conv_groups
                )
...
class ResnetFeatureExtractionChModel(nn.Module):
    def __init__(
        ...
        if learn_norm_input:
            if self.conv1d:
                self.norm_input = Fp32InstanceNorm(in_channels, affine=True)
            else:
                self.norm_input = Fp32InstanceNorm2d(in_channels, affine=True)
        else:
            self.norm_input = None
...
class ResnetFeatureExtractionChMultiModel(nn.Module):
    """
    This model is used to extract features from multiple input
    """
    ...
    def __init__(
        ...
        learn_norm_input: bool = False,
        conv1d: bool = True,
        inner_channels_edges: List[int] = [256],
    ):
        super().__init__()

        for i in range(len(conv_layers)):
            assert conv_layers[i][0] % (len(inner_channels_edges) + 1) == 0
            conv_layers[i] = list(conv_layers[i])
            conv_layers[i][0] = conv_layers[i][0] // (len(inner_channels_edges) + 1)

        self.inputs_edges = split_channel_by_edges(
            channels=in_channels, edges=inner_channels_edges
        )
        self.m_inputs = nn.ModuleList()
        for d in range(len(inner_channels_edges) + 1):
            self.m_inputs.append(
                ResnetFeatureExtractionChModel(
                    in_channels=len(self.inputs_edges[d]),
                    conv_layers=conv_layers,
                    dropout=dropout,
                    mode=mode,
                    conv_bias=conv_bias,
                    conv_groups=conv_groups,
                    learn_norm_input=learn_norm_input,
                    conv1d=conv1d,
                )
            )
    ...
    def forward(self, x):
        feats_cat = []
        for i, conv in enumerate(self.m_inputs):
            feats = conv(x[:, self.inputs_edges[i]])
            feats_cat.append(feats)
        feats_cat = torch.cat(feats_cat, dim=1)
        return feats_cat


class ConvFeatureExtractionChMultiModel(nn.Module):
    """
    This model is used to extract features from multiple input
    """
...
class Conv2DFeatureExtractionChMultiModel(ConvFeatureExtractionChMultiModel):
    """
    This model is used to extract features from multiple input
    """
    ...
    def __init__(
        ...
        super(ConvFeatureExtractionChMultiModel, self).__init__()
        ...
        for d in range(len(inner_channels_edges) + 1):
            self.m_inputs.append(
                Conv2DFeatureExtractionChModel(
                    in_channels=len(self.inputs_edges[d]),
                    conv_layers=conv_layers,
                    dropout=dropout,
                    mode=mode,
...
class GlobalInstanceNorm1d(nn.Module):
    def __init__(self, num_channels, eps=1e-5, affine=True):
        super().__init__()
        self.eps = eps
        self.affine = affine
        if self.affine:
            # Single scale and bias per channel (optional: could make it per instance)
            self.weight = nn.Parameter(torch.ones(1, num_channels, 1))  # broadcastable
            self.bias = nn.Parameter(torch.zeros(1, num_channels, 1))  # broadcastable
