...
def degree_order_to_index(order, degree, ordering=DEFAULT_ORDERING):
    ...
    def sid_idx(n, m):
        idx_order = [
            1+i*2 for i in range(n)] + [n*2] + list(reversed([i*2 for i in range(n)]))
    ...
    def fm_idx(n, m):
        if n == 1:
            ...
            idx_order = list(
                reversed([2*(i+1) for i in range(n)])) + [0] + [1+i*2 for i in range(n)]
        return idx_order[m+n] + n**2
...
def spherical_harmonic_mn(order, degree, phi, nu, normalization=DEFAULT_NORMALIZATION):
    ...
    norm = normalization_factor(degree_order_to_index(
        order, degree), normalization=normalization)
    sph = (-1)**degree * norm * \
...
def spherical_harmonics(position, max_order, ordering=DEFAULT_ORDERING, normalization=DEFAULT_NORMALIZATION):
    ...
    for i in range(num_channels):
        ...
        output[i] = spherical_harmonic_mn(
            order, degree, position.phi, position.nu, normalization)
    return output
