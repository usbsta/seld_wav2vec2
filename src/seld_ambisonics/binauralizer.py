...
from seld_ambisonics.decoder import AmbiDecoder
from seld_ambisonics.hrir import CIPIC_HRIR
from seld_ambisonics.position import MovingSource, Position, PositionalSource
from seld_ambisonics.tdesigns import get_tDesign
    ...
    def binauralize(self, sources):
        ...
        for src in sources:
            for i, mic in enumerate(self.mic_pos):
                dist = np.sqrt(
                    ((src.position.coords('cartesian') - mic.coords('cartesian'))**2).sum())
                mic_vec = mic.coords('cartesian')
                in_vec = mic_vec - src.position.coords('cartesian')
                cos_angle = (mic_vec * in_vec).sum() / \
                    ((mic_vec**2).sum() * (in_vec**2).sum())**0.5
...
class AmbisonicDecoder(object):
    def __init__(self, ambi_format, mic_pos, method='projection', td_order=5, **kwargs):
        ...
        if self.method == 'pseudoinv':
            self.speaker_pos = list(map(lambda x: Position(
                x[0], x[1], x[2], 'cartesian'), get_tDesign(td_order)))
            # self.speaker_pos = list(map(lambda x: Position(x[0], x[1], x[2], 'cartesian'), get_tDesign(self.fmt.order)))
        elif self.method == 'projection':
            speakers_phi = (2. * np.arange(2*self.fmt.num_channels) /
                            float(2*self.fmt.num_channels) - 1.) * np.pi
            self.speaker_pos = list(map(lambda x: Position(
                x, 0, self.fmt.radius, 'polar'), speakers_phi))
        else:
            raise ValueError(
                'Unknown decoding method. Options: projection and pseudoinv')
        self.n_speakers = len(self.speaker_pos)
        self.ambi_decoder = AmbiDecoder(
            self.speaker_pos, self.fmt, method=self.method)
    ...
    def binauralize(self, ambi):
        ...
        sources = list(map(lambda i: PositionalSource(
            speakers[i], self.speaker_pos[i], self.fmt.sample_rate), range(self.n_speakers)))
...
class VirtualStereoMic(object):
    ...
    def binauralize(self, sources):
        ...
        for src in sources:
            l_dist = np.sqrt(
                ((src.position.coords('cartesian') - self.lmic_pos.coords('cartesian'))**2).sum())
            r_dist = np.sqrt(
                ((src.position.coords('cartesian') - self.rmic_pos.coords('cartesian'))**2).sum())

            # Time delay
            l_delay, r_delay = int(
                l_dist / C * src.sample_rate), int(r_dist / C * src.sample_rate)

            # Attenuation is frequency dependent, but lets simplify.
            l_attn, r_attn = 1 / (1. + l_dist), 1 / (1. + r_dist)

            l_signal += l_attn * \
                shift(src.signal, l_delay, cval=0.) / len(sources)
            r_signal += r_attn * \
                shift(src.signal, r_delay, cval=0.) / len(sources)
    ...
    def binauralize_frame(self, sources, output, frame_no):
        ...
        for src in sources:
            ...
            if frame_no-l_delay >= 0:
                output[frame_no, 0] += l_attn * \
                    src.signal[frame_no-l_delay] / len(sources)
            if frame_no-r_delay >= 0:
                output[frame_no, 1] += r_attn * \
                    src.signal[frame_no-r_delay] / len(sources)
...
class Convolvotron(object):
    ...
    def binauralize(self, sources):
        ...
        for src in sources:
            ...
            left_signal = np.convolve(
                src.signal, np.flip(left_hrir, axis=0), 'valid')
            right_signal = np.convolve(
                src.signal, np.flip(right_hrir, axis=0), 'valid')
    ...
    def binauralize_frame(self, sources, output, frame_no):
        ...
        for src in sources:
            ...
            output[frame_no, 0] = (
                src.signal[i_start:i_end] * left_hrir[-i_range:]).sum()
            output[frame_no, 1] = (
                src.signal[i_start:i_end] * right_hrir[-i_range:]).sum()
...
class SourceBinauralizer(object):
    ...
    def binauralize(self, sources):
        ...
        assert isinstance(sources, list) and all(
            [isinstance(src, PositionalSource) for src in sources])
        assert all(
            [src.sample_rate == sources[0].sample_rate for src in sources])
...
class AmbisonicBinauralizer(object):
    def __init__(self, ambi_format, method='projection', use_hrtfs=False, cipic_dir=None):
        self.source_bin = SourceBinauralizer(
            cipic_dir=cipic_dir, use_hrtfs=use_hrtfs)
        self.fmt = ambi_format
        self.method = method

        # Initialize speakers
        if self.method == 'pseudoinv':
            self.speaker_pos = map(lambda x: Position(
                x[0], x[1], x[2], 'cartesian'), get_tDesign(self.fmt.order))
...
class DirectAmbisonicBinauralizer(object):
    def __init__(self, ambi_format, method='projection'):
        ...
        self.ear_pos = [Position(0, 0.1, 0, 'cartesian'),
                        Position(0, -0.1, 0, 'cartesian')]
        self.ambi_decoder = AmbiDecoder(
            self.ear_pos, self.fmt, method=self.method)
...
def test_convolvotron():
    from seld_ambisonics.iolib.audio import load_wav, save_wav
    convolvotron = Convolvotron('hrtfs/cipic_subj3')
    mono, rate = load_wav('wav_test/piano.wav')
    mono = mono[:, 0]

    positions = [[float(num) for num in l.strip().split()]
                 for l in open('wav_test/piano_stat_position.txt', 'r')]
    ...
    positions = [[float(num) for num in l.strip().split()]
                 for l in open('wav_test/piano_mov_position.txt', 'r')]
    positions = [Position(p[0], p[1], p[2], 'polar') for p in positions]
...
def test_source_binauralizer():
    from seld_ambisonics.iolib.audio import load_wav, save_wav
    from seld_ambisonics.iolib.position import read_position_file
    ...
    positions, wav_fns, _, sample_ids = read_position_file(
        sample+'-position.txt')
...
def test_ambisonics_binauralizer():
    from seld_ambisonics.common import AmbiFormat
    from seld_ambisonics.iolib.audio import load_wav, save_wav
