...
from seld_ambisonics.position import MovingSource, Position
...
class SphericalAmbisonicsVisualizer(object):
    def __init__(self, data, rate=22050, window=0.1, angular_res=2.0):
        ...
        mesh_p = [Position(phi, nu, 1., 'polar') for phi, nu in zip(
            self.phi_mesh.reshape(-1), self.nu_mesh.reshape(-1))]

        # Setup decoder
        ambi_order = np.sqrt(data.shape[0]) - 1
        self.decoder = AmbiDecoder(mesh_p, AmbiFormat(
            ambi_order=ambi_order, sample_rate=rate), method='projection')
...
class SphericalSourceVisualizer(object):
    def __init__(self, position_fn, duration, rate=10., angular_res=5):
        ...
        self.sources = [MovingSource(
            np.zeros((self.nframes,)), positions[src_id], rate) for src_id in source_ids]
        self.cur_frame = -1
...
def ambix_emd(ambi1, ambi2, rate, ang_res=20):
    ...
    ambi1Vis = SphericalAmbisonicsVisualizer(
        ambi1, rate, window=0.1, angular_res=ang_res)
    ambi2Vis = SphericalAmbisonicsVisualizer(
        ambi2, rate, window=0.1, angular_res=ang_res)
...
def test_emd():
    from seld_ambisonics.iolib.audio import load_wav
    ...
    ambiVis = SphericalAmbisonicsVisualizer(
        data, rate, window=0.1, angular_res=ang_res)
