...
def preprocess_waves_metadata(
    ...
    for csv_filename in tqdm(csv_files):
        ...
        if vizualize_figs:
            ...
            for i in range(num_classes):
                axs[1, 0].plot(output_ele[i].T, label=f"pool-{i}", linewidth=2.5)
            axs[2, 0].set_title(f"ele 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 0].plot(array_ele[i], label=f"{i}", linewidth=2.5)
            ...
            for i in range(num_classes):
                axs[1, 1].plot(output_azi[i].T, label=f"pool-{i}", linewidth=2.5)
            axs[2, 1].set_title(f"azi 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 1].plot(array_azi[i], label=f"{i}", linewidth=2.5)
            ...
            for i in range(num_classes):
                axs[1, 2].plot(i * output_class[i].T, label=f"pool-{i}", linewidth=2.5)
            axs[2, 2].set_title(f"class 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 2].plot(i * array_class[i].T, label=f"{i}", linewidth=2.5)
        ...
        with h5py.File(save_filename, "w") as f:
            ...
            f.create_dataset("sed_labels_100ms", data=orig_class)
...
@hydra.main(
    version_base=None, config_path=f"{ROOT_DIR}/conf", config_name="config_ft_tau2019"
)
def finetuning_preprocess_data_tau2019(cfg: DictConfig) -> None:
    ...
    if spec_transform is not None:
        save_folder = (
            f"{params['save_folder']}/tau_2019_foa_ft_val_split_xyz_cart_20mst_spec_hdf"
        )
        manifest_folder = (
            f"{params['manifest_folder']}/tau_2019_foa_ft_"
            f"val_split_xyz_cart_20mst_spec_hdf"
        )
    else:
        save_folder = (
            f"{params['save_folder']}/tau_2019_foa_ft_val_split_xyz_cart_20mst_hdf"
        )
        manifest_folder = (
            f"{params['manifest_folder']}/tau_2019_foa_ft_val_split_xyz_cart_20mst_hdf"
        )
...
if __name__ == "__main__":
    finetuning_preprocess_data_tau2019()
