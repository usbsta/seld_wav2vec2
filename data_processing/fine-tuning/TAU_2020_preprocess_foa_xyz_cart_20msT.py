...
logger = logging.getLogger("preprocessing-ft-tau2020")
...
def get_original_DOA_100ms(df, num_classes):
    ele = np.zeros((num_classes, max(df["frame_number"])))
    azi = np.zeros((num_classes, max(df["frame_number"])))
    ele[:] = np.nan
    azi[:] = np.nan
    ...
    for csv_filename in tqdm(csv_files):
        ...
        for class_i in unique_classes:
            ...
            for i in range(len(df_class_index)):
                ...
                ele_array = [ele] * (end - start)
                azi_array = [azi] * (end - start)
        ...
        if vizualize_figs:
            orig_ele, orig_azi = get_original_DOA_100ms(df, num_classes=num_classes)
            orig_class = get_array_class(orig_ele, orig_azi)
            df_array_class = get_array_class(df_array_ele, df_array_azi)
        ...
        for j, (ele, azi) in enumerate(zip(df_split_array_ele, df_split_array_azi)):
            ...
            if (input_length >= MIN_LENGTH) or (not train):
                ...
                if np.min(output_sph) < -180.0:
                    output_sph[output_sph < -180.0] = (
                        output_sph[output_sph < -180.0] + 360
                    )
                ...
                if vizualize_figs:
                    ...
                    output_ele_list.append(output_ele)
                    output_azi_list.append(output_azi)
        ...
        if vizualize_figs:
            output_ele_list = np.hstack(output_ele_list)
            output_azi_list = np.hstack(output_azi_list)
            output_class_list = np.hstack(output_class_list)
            ...
            axs[1, 0].set_title("ele pool 20ms")
            for i in range(num_classes):
                axs[1, 0].plot(output_ele_list[i].T, label=f"pool-{i}", linewidth=2.5)
            axs[2, 0].set_title(f"ele 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 0].plot(df_array_ele[i], label=f"{i}", linewidth=2.5)

            # AZI
            axs[0, 1].set_title("orig-azi 100ms")
            for i in range(num_classes):
                axs[0, 1].plot(orig_azi[i], label=f"orig-{i}", linewidth=2.5)
            axs[1, 1].set_title("azi pool 20ms")
            for i in range(num_classes):
                axs[1, 1].plot(output_azi_list[i].T, label=f"pool-{i}", linewidth=2.5)
            axs[2, 1].set_title(f"azi 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 1].plot(df_array_azi[i], label=f"{i}", linewidth=2.5)

            # CLASS
            axs[0, 2].set_title("orig-class 100ms")
            for i in range(num_classes):
                axs[0, 2].plot(i * orig_class[i].T, label=f"orig-{i}", linewidth=2.5)
            axs[1, 2].set_title("class pool 20ms")
            for i in range(num_classes):
                axs[1, 2].plot(
                    i * output_class_list[i].T, label=f"pool-{i}", linewidth=2.5
                )
            axs[2, 2].set_title(f"class 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 2].plot(i * df_array_class[i].T, label=f"{i}", linewidth=2.5)
...
def finetuning_preprocess_data_tau2020(cfg: DictConfig) -> None:
    ...
    if spec_transform is not None:
        save_folder = (
            f"{params['save_folder']}/tau_nigens_2020_foa_ft_chunk{seldnet_window}"
            f"_stride{stride}_val_split_xyz_cart_20mst_spec"
        )
        manifest_folder = (
            f"{params['manifest_folder']}/tau_nigens_2020_foa_ft_chunk{seldnet_window}"
            f"_stride{stride}_val_split_xyz_cart_20mst_spec"
        )
    else:
        save_folder = (
