    ...
    for csv_filename in tqdm(csv_files):
        ...
        for i in range(len(unique_classes)):
            if np.isnan(df_array_ele[i].values).any():
                sel_df_array_ele = df_array_ele[i][~mask_ele[i]].interpolate(
            ...
            if np.isnan(df_array_azi[i].values).any():
                sel_df_array_azi = df_array_azi[i][~mask_azi[i]].interpolate(
                    method=INTERP_FUN, limit_area="inside"
                )
        ...
        if vizualize_figs:
            ...
            for i in range(num_classes):
                axs[1, 0].plot(
                    output_ele_list[i].T, label=f"interp-pool-{i}", linewidth=2.5
                )
            axs[2, 0].set_title(f"interp-ele 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 0].plot(df_array_ele[i], label=f"interp-{i}", linewidth=2.5)
            ...
            for i in range(num_classes):
                axs[1, 1].plot(
                    output_azi_list[i].T, label=f"interp-pool-{i}", linewidth=2.5
                )
            axs[2, 1].set_title(f"interp-azi 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 1].plot(df_array_azi[i], label=f"interp-{i}", linewidth=2.5)
            ...
            for i in range(num_classes):
                axs[1, 2].plot(
                    i * output_class_list[i].T, label=f"interp-pool-{i}", linewidth=2.5
                )
            axs[2, 2].set_title(f"interp-class 1/{FS_TARGET}s")
            for i in range(num_classes):
                axs[2, 2].plot(
                    i * df_array_class[i].T, label=f"interp-{i}", linewidth=2.5
                )
...
def finetuning_preprocess_data_tau2020(cfg: DictConfig) -> None:
    ...
    if spec_transform is not None:
        save_folder = (
            ...
            f"_stride{stride}_val_split_xyz_cart_20mst_interp_spec"
        )
        manifest_folder = (
    ...
    assert list(unique_classes_dict.values()) == unique_classes
