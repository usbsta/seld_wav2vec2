...
from fairseq.utils import import_user_module, move_to_cuda
...
logger = logging.getLogger("evaluate-thr-2019")


def evaluate_list_thr(
    ckpt_path, batch_size, output_path, user_dir, doa_size=3, data=None
):
    Arg = namedtuple("Arg", ["user_dir"])
    arg = Arg(user_dir.__str__())
    import_user_module(arg)
    ...
    task.cfg.spec_augment = False
    task.load_dataset("valid")

    logger.info("iterating over data")
    batch_itr = DataLoader(
        task.dataset("valid"),
    ...
    for thr in thr_list:
        ...
        for i in range(len(class_labels_list)):
            ...
            class_labels[class_pad_mask] = torch.tensor(0.0).to(class_labels.device)
        ...
        doa_metric = compute_doa_scores_regr_xyz(doa_pred, doa_gt, sed_pred, sed_gt)
    ...
    logger.info(f"best seld-score: {seld_score_list[min_x]}")

    thr_best = thr_list[min_x]
    logger.info(f"best threshold: {thr_best}")

    evaluate(
        ckpt_path,
        "test",
        batch_size,
        f"{output_path}/base_metrics.json",
        user_dir,
        doa_size,
        data,
        thr=0.5,
    ...
    evaluate(
        ...
        "test",
        batch_size,
        f"{output_path}/best_metrics.json",
        user_dir,
        doa_size,
        data,
        thr=thr_best,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_path", required=True, help="path to model checkpoint file."
    )
    parser.add_argument(
        "--batch_size", default=64, type=int, help="max number of tokens in each batch."
    )
    parser.add_argument(
        "--output_path", default="reports", type=str, help=("path output metrics file.")
    )
    parser.add_argument(
        "--user_dir",
        default="src/seld_wav2vec2",
        type=str,
        help=("path to a python module containing custom extensions."),
    )
